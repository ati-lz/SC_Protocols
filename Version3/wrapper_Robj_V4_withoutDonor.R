...
main <- function(hsapExp,mmusExp,hnReads,hnUMI,hnGene,hnFeatures,mnReads,mnUMI,mnGene,mnFeatures, species, 
  ...
  for (sample in 1:number_of_samples){
    ...
    sample.hsap.exon.reads <- data.frame(nExonReads=sample.hfeatures.file[which(sample.hfeatures.file$type == "Exon"), "N"], cellID=create_cell_IDs(as.character(sample.hfeatures.file[which(sample.hfeatures.file$type == "Exon"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID))#; colnames(sample.exon.reads) <- "nExonReads" 
    sample.hsap.intron.reads <- data.frame(nIntronReads=sample.hfeatures.file[which(sample.hfeatures.file$type == "Intron"), "N"], cellID = create_cell_IDs(as.character(sample.hfeatures.file[which(sample.hfeatures.file$type == "Intron"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID)) 
    sample.hsap.intergenic.reads <- data.frame(nIntergenicReads=sample.hfeatures.file[which(sample.hfeatures.file$type == "Intergenic"), "N"], cellID = create_cell_IDs(as.character(sample.hfeatures.file[which(sample.hfeatures.file$type == "Intergenic"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID)) 
    sample.hsap.Unmapped.reads <- data.frame(nUnmappedReads=sample.hfeatures.file[which(sample.hfeatures.file$type == "Unmapped"), "N"], cellID = create_cell_IDs(as.character(sample.hfeatures.file[which(sample.hfeatures.file$type == "Unmapped"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID))
    sample.hsap.Ambiguity.reads <- data.frame(nAmbiguityReads=sample.hfeatures.file[which(sample.hfeatures.file$type == "Ambiguity"), "N"], cellID = create_cell_IDs(as.character(sample.hfeatures.file[which(sample.hfeatures.file$type == "Ambiguity"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID)) 
    ...
    sample.mmus.exon.reads <- data.frame(nExonReads=sample.mfeatures.file[which(sample.mfeatures.file$type == "Exon"), "N"], cellID=create_cell_IDs(as.character(sample.mfeatures.file[which(sample.mfeatures.file$type == "Exon"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID))#; colnames(sample.exon.reads) <- "nExonReads" 
    sample.mmus.intron.reads <- data.frame(nIntronReads=sample.mfeatures.file[which(sample.mfeatures.file$type == "Intron"), "N"], cellID = create_cell_IDs(as.character(sample.mfeatures.file[which(sample.mfeatures.file$type == "Intron"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID)) 
    sample.mmus.intergenic.reads <- data.frame(nIntergenicReads=sample.mfeatures.file[which(sample.mfeatures.file$type == "Intergenic"), "N"], cellID = create_cell_IDs(as.character(sample.mfeatures.file[which(sample.mfeatures.file$type == "Intergenic"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID)) 
    sample.mmus.Unmapped.reads <- data.frame(nUnmappedReads=sample.mfeatures.file[which(sample.mfeatures.file$type == "Unmapped"), "N"], cellID = create_cell_IDs(as.character(sample.mfeatures.file[which(sample.mfeatures.file$type == "Unmapped"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID))
    sample.mmus.Ambiguity.reads <- data.frame(nAmbiguityReads=sample.mfeatures.file[which(sample.mfeatures.file$type == "Ambiguity"), "N"], cellID = create_cell_IDs(as.character(sample.mfeatures.file[which(sample.mfeatures.file$type == "Ambiguity"), "RG"]), id.type = "cell_Barcode",tech = technology, lib = sample.ID)) 
    ...
    print(class(sample.hfeatures.file[which(sample.hfeatures.file$type == "Exon"), "RG"]))
