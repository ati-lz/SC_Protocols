...
main <- function(hsapExp, output_SCEobj, technology) {
  ...
  DS.ranges <- names(init.hsapExp.obj$readcount$exon$downsampling) #change ds.data.mixed to init.hsapExp.obj
  ...
  for (sample in 1:number_of_samples){
    ...
    for (range in 1:length(DS.ranges)){
      Reads.mat <- as.data.frame(as.matrix(sample.hsapExp.obj$readcount$exon$downsampling[[range]]))
  ...
  save(output.readcount.umicount.joint.mats, file = paste(output_SCEobj,"/", technology,".hsap.full.SCE.jointDSmat.exonReads.Robj", sep = ""))
