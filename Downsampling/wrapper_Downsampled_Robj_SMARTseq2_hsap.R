...
main <- function(hsapExp, output_SCEobj, technology) {
  ...
  DS.ranges <- names(init.hsapExp.obj$readcount$inex$downsampling) #change ds.data.mixed to init.hsapExp.obj
  ...
  save(output.readcount.umicount.joint.mats, file = paste(output_SCEobj,"/", technology,".hsap.full.SCE.jointDSmat.2.Robj", sep = ""))
