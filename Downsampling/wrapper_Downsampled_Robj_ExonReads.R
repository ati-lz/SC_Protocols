...
main <- function(hsapExp, output_SCEobj, technology) {
  ...
  DS.ranges <- names(init.hsapExp.obj$umicount$exon$downsampling) #change ds.data.mixed to init.hsapExp.obj
  ...
  for (sample in 1:number_of_samples){
    ...
    for (range in 1:length(DS.ranges)){
      ...
      #print(class(sample.hsapExp.obj$umicount$exon$downsampling[[range]][[1]]))
      #print(dim(sample.hsapExp.obj$umicount$exon$downsampling[[range]][[1]]))
      mat <- as.data.frame(as.matrix(sample.hsapExp.obj$umicount$exon$downsampling[[range]]))
  ...
  save(final.sample.merged.mat, file = paste(output_SCEobj,"/", technology,".hsap.full.SCE.jointDSmat.exonReads.Robj", sep = ""))
